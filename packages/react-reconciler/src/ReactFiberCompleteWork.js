...
function completeWork(
  current: Fiber | null, //! 上次更新好的Fiber（即当前页面所对应的）
  workInProgress: Fiber, //! 内存构建中的Fiber
  renderLanes: Lanes,
): Fiber | null {
  ...
  switch (workInProgress.tag) {
    ...
    case HostComponent: {
      ...
      if (current !== null && workInProgress.stateNode != null) {
        //! update 的情况 
        //! 这一步执行完后Fiber 上的updateQueue 已经挂载为最新的props 
        updateHostComponent(current, workInProgress, type, newProps);
        ...
        if (wasHydrated) {
          ...
          //! 插入后完整的DOM 赋值给Fiber.stateNode (即存储当前Fiber 对应的DOM)
