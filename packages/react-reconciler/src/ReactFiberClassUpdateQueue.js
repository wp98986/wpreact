...
export function initializeUpdateQueue<State>(fiber: Fiber): void {
  ...
  const queue: UpdateQueue<State> = {
    baseState: fiber.memoizedState, // 组件当前的state状态（旧的state）没更新前的state ，新的state 计算基于这个
    firstBaseUpdate: null, // 本次更新前已保存的update 是个链表，这是链表头
    lastBaseUpdate: null,//  本次更新前已保存的update 是个链表，这是链表尾
    shared: {
      pending: null, // 触发更新时产生的update 保存这shared.pending 中，是个单向环装链表（新的state但还没有应用）
      lanes: NoLanes,
      hiddenCallbacks: null,
    },
    callbacks: null, // 存储 update.callback!==null 的update
  };
  fiber.updateQueue = queue; // 在Fiber挂上updateQueue
}
...
export function enqueueUpdate<State>(
  //! 该方法将一个update 加入fiber的update 队列
  fiber: Fiber, // 要更新的fiber
  update: Update<State>, // update实例
  lane: Lane,
): FiberRoot | null {
  const updateQueue = fiber.updateQueue; //! 获取到当前Fiber上的更新队列
  if (updateQueue === null) {
  ...
  if (isUnsafeClassRenderPhaseUpdate(fiber)) {
    ...
    //! 获取已产生的更新的单向环形链表（表示即要被更新到UI），
    const pending = sharedQueue.pending;
    if (pending === null) {
      // This is the first update. Create a circular list.
      update.next = update;
    } else { 
      //! 如果有已经产生的链表，将新加入的update 插入链表头。
      update.next = pending.next;
      pending.next = update;
    }
    sharedQueue.pending = update; // 重新保存更新链表
    ...
    //! 这里先从当前fiber向上找到root节点，并标记更新优先级
