...
export function renderWithHooks<Props, SecondArg>(
  ...
  if (__DEV__) {
    ...
    //! 根据组件mount/update 使用对应的dispatcher 
  ...
  //! 直接执行函数组件，执行的过程中会触发state 的计算。
  //! 上面已经确定了组件mount还是update, 对应组件里的useState触发也会调用不同的dispather 计算新的state。 
...
function updateWorkInProgressHook(): Hook {
  ...
  if (nextWorkInProgressHook !== null) {
    ...
    if (workInProgressHook === null) {
      // This is the first hook in the list.
      //! 将构建的hook池 赋给Fiber.memoizedState 
...
function updateReducer<S, I, A>(
  ...
  //! update阶段 每个useState触发时，找到当前hook,并克隆。移动指针到下一个hook。
  //! 构建新的hook池  
  ...
  //! 这里可以看出。useReducer(reducer,initState) 中reducer 是可以随时变化的
  queue.lastRenderedReducer = reducer;

  const current: Hook = (currentHook: any); //! 上次的hook
...
function updateState<S>(
  //! useState 源码  本质和useReducer 一样，执行了updateReducer
  initialState: (() => S) | S,
): [S, Dispatch<BasicStateAction<S>>] {
...
function pushEffect(
  ...
  let componentUpdateQueue: null | FunctionComponentUpdateQueue =
    (currentlyRenderingFiber.updateQueue: any);
  if (componentUpdateQueue === null) {
    //! 如果fiber 上没有更新队列则创建更新队列 
    componentUpdateQueue = createFunctionComponentUpdateQueue();
    currentlyRenderingFiber.updateQueue = (componentUpdateQueue: any);
    //! 将当前的effect 加入更新，更新是个环状链表  
...
function updateEffectImpl(
  ...
  if (currentHook !== null) {
    ...
    if (nextDeps !== null) {
      ...
      //! 判断依赖是否有改变，即使不变也要重新加入effects链表 ,因为是新的链表，必须保证顺序一致性
...
function mountCallback<T>(callback: T, deps: Array<mixed> | void | null): T {
  const hook = mountWorkInProgressHook();
  const nextDeps = deps === undefined ? null : deps;
  //! useCallback 将整个回调函数作为value 保存到memoizedState 
...
function updateCallback<T>(callback: T, deps: Array<mixed> | void | null): T {
  ...
  if (nextDeps !== null) {
    ...
    if (areHookInputsEqual(nextDeps, prevDeps)) {
      //! 比较前后依赖，如果没有变化，则返回上一次的valu 
      return prevState[0];
    }
  }
  //! 依赖有变化则将新的callback作为value 并返回 
  hook.memoizedState = [callback, nextDeps];
  return callback;
}
...
function dispatchSetState<S, A>(
  fiber: Fiber, //! 当前的Fiber
  queue: UpdateQueue<S, A>, //! hook.queue
  action: A,
): void {
  //! 为state 更新绑定的dispatch ,setState的更新调用这个dispatch 产生新的state 
  if (__DEV__) {
    if (typeof arguments[3] === 'function') {
      console.error(
        "State updates from the useState() and useReducer() Hooks don't support the " +
  ...
  if (isRenderPhaseUpdate(fiber)) {
    ...
    if (
      ...
      if (lastRenderedReducer !== null) {
        ...
        try {
          const currentState: S = (queue.lastRenderedState: any);//! 上次的state
          const eagerState = lastRenderedReducer(currentState, action); //! 基于上次的state计算新的state
    ...
    //! 将新的update 加入队列  并返回root 
    const root = enqueueConcurrentHookUpdate(fiber, queue, update, lane);
    if (root !== null) {
      const eventTime = requestEventTime();
      //! 开启调度更新Fiber  =>  ensureRootIsScheduled() 进入beginwork新一轮render 页面重新渲染
      scheduleUpdateOnFiber(root, fiber, lane, eventTime);
      entangleTransitionUpdate(root, queue, lane);
    }
  }
...
const HooksDispatcherOnMount: Dispatcher = {
  //! mount阶段的hooks dispather
  //!  组件里的useState的触发最终会调用到这里的方法，从而触发state 计算
  readContext,
...
const HooksDispatcherOnUpdate: Dispatcher = {
  //! update阶段的hooks dispather
  //!  组件里的useState的触发最终会调用到这里方法，从而触发state 计算
  readContext,
