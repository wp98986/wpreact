...
function bailoutOnAlreadyFinishedWork(
  ...
  //! 如果children 没有高优先级的任务，则说明child 不用背更新，不需要进行协调。
...
function attemptEarlyBailoutIfNoScheduledUpdate(
  ...
  // ! 不知道上面做了什么，这里面会做clone Fiber操作 来复用Fiber
  // ! 百度说那一堆pusHXXX做了一些处理context 的逻辑，看起来不是啥主逻辑，不重要
  // ! bailoutOnAlreadyFinishedWork 这个方法才是主要的，用来clone 复用fiber
  return bailoutOnAlreadyFinishedWork(current, workInProgress, renderLanes);
}

function beginWork(
  current: Fiber | null, // 当前组件对应上次更新时的Fiber
  workInProgress: Fiber, // 内存中的Fiber
  renderLanes: Lanes,
): Fiber | null {
  //! beginwork 阶段
  if (__DEV__) {
  ...
  if (current !== null) {
    //! 如果存在上次更新时的Fiber ,说明是update. 则需要判定是否可以复用Fiber
    ...
    if (
      ...
      // !如果本次和上次props 不相等，type也不同，则说明不可复用。didReceiveUpdate标记为true
  ...
  workInProgress.lanes = NoLanes;
  //! mount 直接创建Fiber
  //! 常见的组件都会进入 reconcileChildren() 方法
  //! 完成后结束beginwork 阶段 
  switch (workInProgress.tag) {
