...
export function scheduleUpdateOnFiber(
  ...
  if (
    ...
    //! 关键函数：调度更新，会进入performSyncWorkOnRoot/performConcurrentWorkOnRoot 开启协调阶段。
...
function performConcurrentWorkOnRoot(
  ...
  //! 判断进入异步render 还是 同步render
    ? renderRootConcurrent(root, lanes) // 异步
...
function workLoopConcurrent() {
  ...
  //! shouldYield() 表示当前浏览器帧有没有剩余时间，如果没有剩余时间才会去performUnitOfWork，
  //! 如果当前帧有剩余时间会执行GPU渲染工作。 （任务切片相关）
  while (workInProgress !== null && !shouldYield()) {
    // $FlowFixMe[incompatible-call] found when upgrading Flow
    //! performUnitOfWork 的作用是创建下一个Fiber节点，并赋值给workInProgress。和已创建的Fiber链接起来构成Fiber树
    performUnitOfWork(workInProgress);
  }
}
...
function performUnitOfWork(unitOfWork: Fiber): void {
  ...
  //! 双缓存策略 当前屏幕显示内对对应的Fiber的alternate 指向 内存中在构建的Fiber
  //! 反之 内存中正在构建的Fiber 的alternate 指向当前屏幕显示对应的Fiber
  const current = unitOfWork.alternate; // !这里先获取到屏幕显示的对应的Fiber
  setCurrentDebugFiberInDEV(unitOfWork);

  let next;
  if (enableProfilerTimer && (unitOfWork.mode & ProfileMode) !== NoMode) {
    startProfilerTimer(unitOfWork);
    //! 以当前组件上次更新的Fiber和 本次更新要构建的Fiber 为入参，进入beginwork 阶段
    //!  beginwork 生成新的子Fiber ,并赋值给workInProgress.child 作为返回值,unitOfWork就是workInProgress
    next = beginWork(current, unitOfWork, renderLanes);
    stopProfilerTimerIfRunningAndRecordDelta(unitOfWork, true);
  } else {
    next = beginWork(current, unitOfWork, renderLanes);
  }
  //! beginwork作用：传图当前Fiber 创建子Fiber,并连接两个Fiber
  resetCurrentDebugFiberInDEV();
  unitOfWork.memoizedProps = unitOfWork.pendingProps;
  if (next === null) {
    // If this doesn't spawn new work, complete the current work.
    //! 如果当前Fiber没有child,结束递的阶段，已当前Fiber作为入参开始归的阶段。
    //!  没有next则说明递阶段结束，开始归的阶段 ，
    //! 对当前Fiber执行完完 completeUnitOfWork 后则进入其兄弟Fiber的递，否则进入父节点的递
    //! complete 完成后，归的阶段完成，
    completeUnitOfWork(unitOfWork);
  } else {
    //! 将创建的子Fiber 置为workInProgress 继续深度遍历，进行下一次performUnitOfWork
    workInProgress = next;
  }
...
function completeUnitOfWork(unitOfWork: Fiber): void {
  ...
  do {
    ...
    // nothing should rely on this, but relying on it here means that we don't
    // need an additional field on the work in progress.
    const current = completedWork.alternate; //! 当前Fiber 在当前页面对应的Fiber
    const returnFiber = completedWork.return; //! Fiber 的父节点
    ...
    if ((completedWork.flags & Incomplete) === NoFlags) {
      ...
      if (
        ...
        //!  进入completeWork() 
        //! 里面会根据mount/update update时，更新Fiber上的updateQueue,mount 时创建DOM
        //! completeWork 完成后，归的阶段完成。
...
function commitRootImpl(
  ...
  //! beforeMutation 之前的准备阶段，调度上次剩余的 useEffect 
  do {
    ...
    //!  触发上次更新剩余的 生命周期函数和hooks(useEffect) 
    flushPassiveEffects();
  ...
  //!  上面属于beforeMutation之前的准备阶段，这个阶段会调度useEffect
  ...
  if (subtreeHasEffects || rootHasEffect) {
    ...
    //!  beforeMutation 阶段（操作DOM之前） 这个阶段会调度useEffect.调用getSnapshotBeforeUpdate
    ...
    //!  mutation 阶段 （操作DOM） 会执行上个阶段的useLayoutEffect 的销毁函数
    //!  类组件这个阶段会清除上次的ref
    ...
    //!  layout 阶段 （操作DOM之后）
    //! 类组件重新赋值ref 
    //! 函数组件将通过useRef() 创建并返回新的ref beginwork阶段触发
...
function flushPassiveEffectsImpl() {
  ...
  //! 执行组件上次render时的 销毁相关生命周期函数和hooks 
  commitPassiveUnmountEffects(root.current);
  //! 执行组件本次render 挂载相关的生命周期函数和hooks 
