...
function safelyAttachRef(current: Fiber, nearestMountedAncestor: Fiber | null) {
  //! 前面mutation阶段清除了ref, 这次layout阶段重新赋值ref 
  try {
...
export function commitBeforeMutationEffects(
  ...
  commitBeforeMutationEffects_begin();//! 主要调用commitBeforeMutationEffectsOnFiber 
...
function commitBeforeMutationEffects_begin() {
  while (nextEffect !== null) {
    ...
    if (
      ...
      commitBeforeMutationEffects_complete(); //! 调用commitBeforeMutationEffectsOnFiber
    }
  }
}
...
function commitBeforeMutationEffectsOnFiber(finishedWork: Fiber) {
  ...
  switch (finishedWork.tag) {
    case FunctionComponent: {
      //! 调度useEffect, 把useEffect产生的生命周期函数加入调度队列，此时不会执行useEffect
    ...
    case ClassComponent: {
      if ((flags & Snapshot) !== NoFlags) {
        if (current !== null) {
          ...
          //! 这里调用getSnapshotBeforeUpdate钩子函数 这是最后访问更新前旧的state和props 的机会，
...
function commitHookEffectListUnmount(
  ...
  const updateQueue: FunctionComponentUpdateQueue | null =
    (finishedWork.updateQueue: any); // 获取的Fiber上的更新队列
  ...
  if (lastEffect !== null) {
    ...
    do {
      ...
      if ((effect.tag & flags) === flags) {
        // Unmount
        const destroy = effect.destroy; //! 获取到销毁函数
        effect.destroy = undefined; //! 将原来的销毁函数全部清除，解决多组件互相影响
      ...
      effect = effect.next; //! 操作下一个Effect
  ...
  if (lastEffect !== null) {
    ...
    do {
      ...
      if ((effect.tag & flags) === flags) {
        ...
        // Mount
        const create = effect.create; //! 获取副作用 cerate 
        if (__DEV__) {
          if ((flags & HookInsertion) !== NoHookEffect) {
            setIsRunningInsertionEffect(true);
          }
        }
        effect.destroy = create(); //! 执行副作用的create，并将返回值赋给effect.destroy
        // useEffect  里的return 就是销毁函数。所以讲create()执行的结果赋给effect.destroy，即存储了销毁方法
        if (__DEV__) {
          if ((flags & HookInsertion) !== NoHookEffect) {
            setIsRunningInsertionEffect(false);
          }
...
function commitMutationEffectsOnFiber(
  ...
  switch (finishedWork.tag) {
    ...
    case SimpleMemoComponent: {
      ...
      if (flags & Update) {
        try {
          //! 调用上个阶段 useLayoutEffect hook的销毁函数 
